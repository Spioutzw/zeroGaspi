...
export default function QuantiteForm({ productName,onFormClose,userId }: { productName: string,onFormClose: () => void,userId: string|undefined }) {
      ...
      if (selectedDate) {
        ...
        if (currentIndex < newDates.length - 1) {
          ...
          try {
            ...
            if(userId) {
              const formattedDates = newDates.map(date => date.toLocaleDateString());
              ...
              console.log(newDates,'newDates')
                if (typeof productName === 'string') {
                  scheduleExpirationNotifications(productName,productId,newDates);
              } else {
                  scheduleExpirationNotifications(productName[0],productId,newDates);
              }
  ...
  return (
    <View style={styles.container}>
      <Controller
        ...
        defaultValue=""
      />
      <Button style={styles.button} mode='elevated' onPress={handleSubmit(onSubmit)} >
        <Text > Soumettre</Text>
      </Button>
